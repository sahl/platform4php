...
class ConditionGreaterEqual extends Condition {
    ...
    public function getAsArray(): array {
        return ['type' => 'GreaterEqual', 'fieldname' => $this->fieldname, 'value' => $this->type->getJSONValue($this->value)];
    }
    
    public function getSQLFragment(): string {
        $sql = $this->type->filterGreaterEqualSQL($this->value);
        if ($sql === false) {
            $this->setNoSQL();
            return 'TRUE';
        }
    ...
    protected function manualMatch(Datarecord $datarecord, bool $is_prefiltered) : bool {
        ...
        return $this->type->filterGreaterEqual($datarecord->getRawValue($this->fieldname), $this->value);
    }
