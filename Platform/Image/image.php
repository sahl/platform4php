...
class Image {
     ...
     * @param boolean $soft_fail If this is true, then the function will return false if an image couldn't be read. Otherwise it will throw an error.
     * @return boolean True if an image was read from the file.
     */
    public function attachFile($file, $soft_fail = false) {
        Errorhandler::checkParams($file, '\\Platform\\File', $soft_fail, 'boolean');
    ...
    public function attachImageData($image) {
        Errorhandler::checkParams($image, 'resource');
        $this->image_data = $image;
    }
     ...
     */
    public function attachToFileAsJPEG($file, $quality = null) {
        Errorhandler::checkParams($file, '\\Platform\\File');
        global $platform_configuration;
        if ($this->image_data == null) trigger_error('Tried to get image without data.', E_USER_ERROR);
        $temp_file = tempnam($platform_configuration['dir_temp'], 'platform_img');
     ...
     * @param \Platform\File $file File object
     */
    public function attachToFileAsPNG($file) {
        Errorhandler::checkParams($file, '\\Platform\\File');
        global $platform_configuration;
        if ($this->image_data == null) trigger_error('Tried to get image without data.', E_USER_ERROR);
        $temp_file = tempnam($platform_configuration['dir_temp'], 'platform_img');
     ...
     */
    public function crop($width, $height, $strategy = self::RESIZE_STRATEGY_FILL) {
        Errorhandler::checkParams($width, 'int', $height, 'int');
     ...
     */
    public function downsize($width, $height, $strategy = self::RESIZE_STRATEGY_FILL) {
        Errorhandler::checkParams($width, 'int', $height, 'int');
        if (! self::isValidStrategy($strategy)) trigger_error('Invalid strategy: '.$strategy, E_USER_ERROR);
        if ($this->image_data == null) trigger_error('Tried operation on image without data.', E_USER_ERROR);
        $current_width = imagesx($this->image_data);
     ...
     * @param int $strategy Resize strategy
     */
    public function resize($width, $height, $strategy = self::RESIZE_STRATEGY_FILL) {
        Errorhandler::checkParams($width, 'int', $height, 'int');
        if (! self::isValidStrategy($strategy)) trigger_error('Invalid strategy: '.$strategy, E_USER_ERROR);
        if ($this->image_data == null) trigger_error('Tried operation on image without data.', E_USER_ERROR);
        $current_width = imagesx($this->image_data);
