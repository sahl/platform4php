 ...
 * Class for logging
 * 
 * @link https://wiki.platform4php.dk/doku.php?id=log_class
 */
...
class Log {
    ...
    public function log() {
        ...
        $output_line = date('Y-m-d H:i:s').'.'.str_pad(microtime(true)*1000%1000,3,'0', STR_PAD_LEFT);
        for ($i = 0; $i < func_num_args(); $i++) {
            $string = func_get_arg($i);
            if ($i < count($this->lineformat)) {
                $output_line .= ' '.str_pad($string, $this->lineformat[$i], ' ', $this->linealign[$i] == 'r' ? STR_PAD_LEFT : STR_PAD_RIGHT);
            } else {
                $output_line .= ' '.$string;
            }
        }
