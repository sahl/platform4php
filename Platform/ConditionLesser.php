...
namespace Platform;
/**
 * Condition class for implementing a lesser condition.
 * 
...
class ConditionLesser extends Condition {
    ...
    public function validate() {
        ...
        if (! $definition) return array('Invalid field '.$this->fieldname.' for lesser condition');
        if ($definition['store_in_database'] === false) return array('Field '.$this->fieldname.' is not stored in database for lesser condition');
        if (! in_array($definition['fieldtype'], static::$valid_field_types)) return array('Field '.$this->field.' does not work with lesser condition');
