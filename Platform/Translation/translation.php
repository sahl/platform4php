...
class Translation {
     ...
     * @return string|boolean Path to file or false if convertion not possible.
     */
    public static function convertURLToServerPath($url) {
        Errorhandler::checkParams($url, 'string');
     ...
     * @return string URL
     */
    public static function convertServerPathToURL($server_path) {
        Errorhandler::checkParams($server_path, 'string');
        $baseurl = $_SERVER['DOCUMENT_ROOT'];
        if (substr($baseurl,-1) == '/') $baseurl = substr($baseurl,0,-1);
     ...
     */
    public static function createUpdateTranslationFilesWithPhrases($original_file, $phrases) {
        Errorhandler::checkParams($original_file, 'string', $phrases, 'array');
        foreach (self::getLanguageKeys(false) as $language_key) {
            ...
            File::ensureFullPath($translation_file, true);
     ...
     */
    public static function createUpdateTranslationFilesFromDirectory($directory) {
        Errorhandler::checkParams($directory, 'string');
    ...
    public static function getConfiguration($key) {
        ...
        return $platform_configuration['translation'][$key];
    }
     ...
     */
    public static function getCSVFileName($language_key) {
     ...
     */
    public static function getDirectoriesFromConfiguration() {
     ...
     */
    public static function getFileListFromDirectory($directory, $extensions = array()) {
        Errorhandler::checkParams($directory, 'string', $extensions, 'array');
     ...
     * @param boolean $include_base_language Decide if the base language should be included in the array
     * @return array
     */
    public static function getLanguageArray($include_base_language = true) {
        $result = array();
     ...
     */
    public static function getTranslationFileFromOriginalFile($original_file, $language_key) {
     ...
     */
    public static function getLanguageKeys($include_base_language = true) {
     ...
     */
    public static function getPhrasesFromFile($original_file) {
        Errorhandler::checkParams($original_file, 'string');
        $ext = File::extractExtension($original_file);
     ...
     */
    public static function getJSFilesForFile($js_file) {
    ...
    public static function isEnabled() {
        global $platform_configuration;
        return is_array($platform_configuration['translation']);
    }
     ...
     */
    public static function loadTranslationsFromCSVFile($csv_file) {
    ...
    public static function renderJSFilesForFile($original_file) {
        if (self::isEnabled()) {
     ...
     */
    public static function replaceStringParameters($string, $replacement_map) {
     ...
     * @param array $phrases
     */
    public static function saveCSVFile($language_key, $phrases) {
        Errorhandler::checkParams($language_key, 'string', $phrases, 'array');
     ...
     */
    public static function saveTranslationFile($translation_file, $language_key, $phrases) {
        $fh = fopen($translation_file, 'w');
        $is_php_file = File::extractExtension($translation_file) == 'php';
     ...
     */
    public static function translateForUser() {
        global $platform_language;
        $args = func_get_args();
     ...
     * @return string Translated phrase or original phrase if no translation found
     */
    public static function translateForInstance() {
        global $platform_language;
        $args = func_get_args();
        if (count($args) < 1) return '';
